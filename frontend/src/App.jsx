...
const AdminPanel = () => {
  const navigate = useNavigate();
  const { jobs, applications, pendingPayments, completedTransactions, isLoggedIn } = useAppContext();
  const [activeTab, setActiveTab] = useState('overview');
  const [deleteConfirm, setDeleteConfirm] = useState(null);

  // Admin authentication - hardcoded for demo
  const isAdmin = isLoggedIn && localStorage.getItem('adminAccess') === 'true';
  ...
  const stats = [
    { label: 'Total Jobs', value: jobs.length, icon: Briefcase, color: 'from-purple-600 to-blue-600' },
    { label: 'Applications', value: applications.length, icon: FileText, color: 'from-blue-600 to-cyan-500' },
    { label: 'Pending Payments', value: pendingPayments.length, icon: CreditCard, color: 'from-orange-500 to-red-500' },
    { label: 'Completed Transactions', value: completedTransactions.length, icon: CheckCircle, color: 'from-green-600 to-emerald-500' },
  ];
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-500">Manage all platform data</p>
          </div>
          <button
            onClick={() => {
        ...
        <div className="flex gap-2 border-b border-gray-200">
          <button
            ...
            className={`px-6 py-3 font-semibold transition-colors ${activeTab === 'overview' ? 'text-purple-600 border-b-2 border-purple-600' : 'text-gray-500 hover:text-gray-700'}`}
          >
            Overview
          </button>
          <button
            onClick={() => setActiveTab('jobs')}
            className={`px-6 py-3 font-semibold transition-colors ${activeTab === 'jobs' ? 'text-purple-600 border-b-2 border-purple-600' : 'text-gray-500 hover:text-gray-700'}`}
          >
            Jobs
          </button>
          <button
            onClick={() => setActiveTab('applications')}
            className={`px-6 py-3 font-semibold transition-colors ${activeTab === 'applications' ? 'text-purple-600 border-b-2 border-purple-600' : 'text-gray-500 hover:text-gray-700'}`}
          >
            Applications
          </button>
          <button
            onClick={() => setActiveTab('payments')}
            className={`px-6 py-3 font-semibold transition-colors ${activeTab === 'payments' ? 'text-purple-600 border-b-2 border-purple-600' : 'text-gray-500 hover:text-gray-700'}`}
          >
            Payments
          </button>
          <button
            onClick={() => setActiveTab('transactions')}
            className={`px-6 py-3 font-semibold transition-colors ${activeTab === 'transactions' ? 'text-purple-600 border-b-2 border-purple-600' : 'text-gray-500 hover:text-gray-700'}`}
          >
            Transactions
          </button>
        </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border p-8 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-purple-50 rounded-xl p-6 border border-purple-100">
                <h3 className="font-bold text-gray-900 mb-2">Active Users</h3>
                <p className="text-3xl font-bold text-purple-600">{Math.floor(Math.random() * 1000) + 100}</p>
              </div>
              <div className="bg-blue-50 rounded-xl p-6 border border-blue-100">
                <h3 className="font-bold text-gray-900 mb-2">Total Revenue</h3>
                <p className="text-3xl font-bold text-blue-600">Rs. {completedTransactions.reduce((sum, t) => sum + (t.amount || 0), 0).toLocaleString()}</p>
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'jobs' && (
          <div className="bg-white rounded-2xl shadow-sm border overflow-hidden">
            <div className="px-8 py-6 border-b">
              <h3 className="font-bold text-gray-800 text-lg">All Jobs</h3>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead className="bg-gray-50 text-gray-500 text-xs uppercase">
                ...
                <tbody className="divide-y">
                  {jobs.map(job => (
                    <tr key={job.id} className="hover:bg-gray-50">
                      <td className="px-8 py-4 font-semibold">{job.title}</td>
                      <td className="px-8 py-4">{job.company}</td>
                      <td className="px-8 py-4">{job.location}</td>
                      <td className="px-8 py-4"><span className="bg-purple-100 text-purple-700 px-3 py-1 rounded-full text-xs font-bold">{job.type}</span></td>
                      <td className="px-8 py-4 text-right">
                        <button
        ...
        {activeTab === 'applications' && (
          <div className="bg-white rounded-2xl shadow-sm border overflow-hidden">
            <div className="px-8 py-6 border-b">
              <h3 className="font-bold text-gray-800 text-lg">All Applications</h3>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead className="bg-gray-50 text-gray-500 text-xs uppercase">
                ...
                <tbody className="divide-y">
                  {applications.map(app => (
                    <tr key={app.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-8 py-4 text-gray-600">{app.date}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {activeTab === 'payments' && (
          <div className="bg-white rounded-2xl shadow-sm border overflow-hidden">
            <div className="px-8 py-6 border-b">
              <h3 className="font-bold text-gray-800 text-lg">Pending Payments</h3>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead className="bg-gray-50 text-gray-500 text-xs uppercase">
                ...
                <tbody className="divide-y">
                  {pendingPayments.map(payment => (
                    <tr key={payment.id} className="hover:bg-gray-50">
                      <td className="px-8 py-4 font-semibold">{payment.jobTitle}</td>
                      <td className="px-8 py-4">{payment.userName}</td>
                      <td className="px-8 py-4 font-bold text-purple-600">Rs. {payment.amount.toLocaleString()}</td>
                      <td className="px-8 py-4"><span className="bg-yellow-100 text-yellow-700 px-3 py-1 rounded-full text-xs font-bold">Pending</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {activeTab === 'transactions' && (
          <div className="bg-white rounded-2xl shadow-sm border overflow-hidden">
            <div className="px-8 py-6 border-b">
              <h3 className="font-bold text-gray-800 text-lg">Completed Transactions</h3>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead className="bg-gray-50 text-gray-500 text-xs uppercase">
                ...
                <tbody className="divide-y">
                  {completedTransactions.map(transaction => (
                    <tr key={transaction.transactionId} className="hover:bg-gray-50">
                      <td className="px-8 py-4 font-semibold">{transaction.jobTitle}</td>
                      <td className="px-8 py-4">{transaction.userName}</td>
                      <td className="px-8 py-4 font-bold text-green-600">Rs. {transaction.amount.toLocaleString()}</td>
                      <td className="px-8 py-4 text-gray-600">{transaction.completedAt}</td>
                      <td className="px-8 py-4"><span className="bg-green-100 text-green-700 px-3 py-1 rounded-full text-xs font-bold">Completed</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="bg-white rounded-2xl p-8 max-w-sm">
              ...
              <div className="flex gap-4">
                <button
                  ...
                  className="flex-1 bg-gray-200 text-gray-800 px-4 py-2 rounded-xl font-bold hover:bg-gray-300"
                >
                  Cancel
                </button>
                <button
